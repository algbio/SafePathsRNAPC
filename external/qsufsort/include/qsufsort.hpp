#ifndef SAFEPATHSRNAPC_QSUFSORT_HPP
#define SAFEPATHSRNAPC_QSUFSORT_HPP
#define KEY(p)          (V[*(p)+(h)])
...
/* Adaptation of qsufsort (http://www.larsson.dogma.net/qsufsort.c) */
...
public:
    ...
    std::vector<int> locate(std::vector<int>& pattern, int *text, int n, int* sa) {
        int lo = 1;
        int hi = n;
        while (lo < hi) {
            int mid = lo + (hi-lo)/2;
            int suffix_pos = sa[mid];

            bool greater_or_equal = false;
            int i;
            for (i = 0; i < pattern.size() && suffix_pos+i < n; ++i) {
        ...
        while (lo < hi) {
            ...
            bool starts_with_path = true;
            for (int i = 0; i < pattern.size() && suffix_pos+i < n; ++i) {
                if (pattern[i] != text[suffix_pos+i]) {
                    starts_with_path = false;
                    break;
                }
            }
            if (!starts_with_path) {
...
#endif //SAFEPATHSRNAPC_QSUFSORT_HPP
